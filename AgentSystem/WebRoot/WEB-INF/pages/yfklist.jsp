...
<div class="container">
	...
	<div class="pagination pagination-centered">
		<ul>
			<li>
				<a href="/yfklist.action?pager.page=1&user.id=${user.id}&user.userCode=${user.userCode}&accountDetail.detailType=<s:property value="accountDetail.detailType"/>&accountDetail.startTime=<s:date name="accountDetail.startTime" format="yyyy-MM-dd"/>&accountDetail.endTime=<s:date name="accountDetail.endTime" format="yyyy-MM-dd"/>">
					首页
				</a>
			</li>
			<!-- 前一段页面 -->
			<s:if test="pager.prevPages!=null"> <!-- 此处的pager.prevPages直接通过调用PageSupport中的getPrevPages()方法取到的值，而没有相应的属性值 -->
				<s:iterator value="pager.prevPages" var="number">
					<li>
						<a href="/yfklist.action?pager.page=<s:property value="#number"/>&user.id=${user.id}&user.userCode=${user.userCode}&accountDetail.detailType=<s:property value="accountDetail.detailType"/>&accountDetail.startTime=<s:date name="accountDetail.startTime" format="yyyy-MM-dd"/>&accountDetail.endTime=<s:date name="accountDetail.endTime" format="yyyy-MM-dd"/>">
			...
			<!-- 尾页 -->
			<li>
				<a href="/yfklist.action?pager.page=<s:property value="pager.pageCount"/>&user.id=${user.id}&user.userCode=${user.userCode}&accountDetail.detailType=<s:property value="accountDetail.detailType"/>&accountDetail.startTime=<s:date name="accountDetail.startTime" format="yyyy-MM-dd"/>&accountDetail.endTime=<s:date name="accountDetail.endTime" format="yyyy-MM-dd"/>">
					尾页
				</a>
			</li>
		</ul>
